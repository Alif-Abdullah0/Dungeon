...
el = tagger(es,'head','<style> body, html {  height: 100%;  margin: 0;}.bg {  background-image: url("base.png");  height: 100%;  background-position: center;  background-repeat: no-repeat;  background-size: cover;} </style>')
html = tagger(el, 'head', "<link rel='stylesheet' type='text/css' href='reg.css'>")
if 'base' in data:
    if data.getvalue('base') == 'o1':
        ...
        link = tagger(inter,'body','<br> <a href="equip.py?equip=Skulmet&web=base.py"> Equip the item? </a> <br>')
        fin = tagger(link,'body','<a href="base.py"> Or Turn Around? </a>')
        print(fin)
    elif data.getvalue('base') == 'o2':
        inter = tagger(html,'body','You take the catacombs door. Not much else to say.')
        fin = tagger(inter,'body','<br> <a href="catacombs.py"> Walk under Paris ... I mean through the catacombs </a>')
        print(fin)
    elif data.getvalue('base') == 'o3':
        inter = tagger(html,'body','You walk through the cathedral doorway. yas ot esle hcum toN.')
        fin = tagger(inter,'body','<br> <a href="cathedral.py"> Enter the chamber </a>')
        print(fin)
    else:
        ...
        print(tagger(inter,'body','<br> <a href="entry.py"> Back to the drawing board. </a>'))
else:
